...
import { UploadPicture, UploadVideo, UploadPost } from "./components/blocks";
...
function App() {
  ...
  const handleEmbedVideoClick = async (url: string) => {
    ...
    if (url && quill) {
      ...
      const videoData = await fetchVideoData(url); // Call fetchVideoData function
      console.log(videoData); // Do something with the video data
    }

    closeModal();
  };

  const handleEmbedPostClick = async (postUrl: string) => {
    const quill = quillRef?.getEditor();
    const range = quill?.getSelection(true);

    if (postUrl && quill) {
      const postId = postUrl.split("/posts/")[1];
      const embedCode = `<iframe src="https://www.facebook.com/plugins/post.php?href=https://www.facebook.com/${postId}" width="500" height="600" style="border:none;overflow:hidden" scrolling="no" frameborder="0" allowTransparency="true" allow="encrypted-media"></iframe>`;
      quill.insertEmbed(range?.index || 0, "post", embedCode);
    }
  ...
  return (
    <>
      ...
      {choice === "social" && (
        <UploadPost
          closeModal={closeModal}
          handleEmbedPostClick={handleEmbedPostClick}
        />
      )}
    </>
  );
